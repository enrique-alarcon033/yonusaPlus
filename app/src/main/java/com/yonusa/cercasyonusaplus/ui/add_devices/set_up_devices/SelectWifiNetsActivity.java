...
import com.yonusa.cercasyonusaplus.ui.add_devices.controller.ListaWifiAdapter;
import com.yonusa.cercasyonusaplus.ui.add_devices.models.ListaWifi;
...
public class SelectWifiNetsActivity extends AppCompatActivity implements SocketTCP.setResultTCP {
                        ...
                        if (!result.contains("NOT-FOUND")) {//Si la respuesta es Success
                            ...
                            runOnUiThread(() -> {
                                if(network_array.length!=0){
                                    ...
                                    Toast.makeText(context, "La  tarjeta wifi no encontro redes inicia nuevamente la configuiración", Toast.LENGTH_LONG).show();
                                }
    ...
    private void networkConstructor(String[] array_networks) {
        ...
        if (array_networks.length>=1) {
            ...
            Toast.makeText(context, "No se encuentran las redes wifi al alcanse o la red wifi no es compatible", Toast.LENGTH_LONG).show();
        }
        loaderInteractionEnable();
